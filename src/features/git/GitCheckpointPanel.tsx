  ...
  const handleRestore = async (cp: GitCheckpoint) => {
    ...
    if (res.data.requireConfirm) {
      ...
      if (forceRes.ok && forceRes.data.success) {
        onToast?.(`已成功还原至快照 (${new Date(cp.timestamp).toLocaleTimeString()})，且已自动备份当前状态。`)
        await loadData()
      } else {
        ...
        {!workspacePath ? (
          ...
          <>
            ...
            <div className="settings-footnote" style={{ marginTop: 10 }}>
              <ShieldCheck size={14} />
              <span>
                TaskWeaver 原生依托 Git Stdio 构建检查点。还原前自动保留当前备份，杜绝静默覆盖或误删工作区数据。
              </span>
            </div>
          </>
        )}
